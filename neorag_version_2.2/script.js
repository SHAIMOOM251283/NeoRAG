...
newSessionBtn.addEventListener('click', async () => {
  if (!confirm("Start a new session?\nCurrent chat and document will be cleared.")) {
    return;
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      addMessage("New session ready. Upload a PDF to begin.", false);
    } else {
      addMessage("Failed to clear session: " + (data.error || "Unknown error"), false);
    }
  } catch (err) {
    addMessage("Network error while clearing session.", false);
  } finally {
    hideLoading();
  }
});
